...
var _user = __webpack_require__(/*! ../../api/user.js */ 306);
var TOKENKEY = 'BLOG_TOKEN';
var _default = {
  namespaced: true,
  ...
  //存放同步操作
  mutations: {
    saveLogin: function saveLogin(state, token) {
      state.token = token;
      uni.setStorage({
        key: TOKENKEY,
        data: token
    ...
    removeLogin: function removeLogin(state) {
      //删除登录状态
      state.token = '';
      uni.removeStorage({
        key: TOKENKEY
...
var _user = _interopRequireDefault(__webpack_require__(/*! ./modules/user */ 305));
//1.导入vue和vuex实例
